...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        ...
        # Verificar se o número de transações excede o limite
        if len(transacoes_hoje) >= self.LIMITE_TRANSACOES_POR_DIA:
...
class Historico:
    ...
    def transacoes_do_dia(self):
        hoje = datetime.now().strftime("%d-%m-%Y")  # Obter a data atual no mesmo formato das transações
        transacoes_do_dia = [t for t in self._transacoes if t["data"].startswith(hoje)]  # Filtrar transações por data
